...
set tag_id = t.id
from (
	select t.id, b.album_cluster from linked.album as b
	left join tag as t on t.value ilike b.description::text || '%%'
	where base_owner is true) as t
where a.album_cluster = t.album_cluster;

---- create linked.asset

with asset_filter as (select b.album_cluster,a.asset_cluster,b.owner_id,b.base_owner,a.id from linked.album as b
	left join tag_asset as ta on b.tag_id = ta."tagsId"
...
final_table as (
	...
	where owner_id is not null and not exists (select 1 from tag_asset where "tagsId" = b.tag_id and "assetsId" = a.id))
...
DECLARE
	...
	a_asset_cluster UUID;
BEGIN
	IF EXISTS (SELECT 1 FROM linked.tag WHERE id = old."tagsId" and base_owner is true) 
	and not exists (select 1 from linked.tag_helper where id=old."assetsId" and status is true and now() - interval '1 second' <= updated) THEN
...
BEGIN
	if TG_OP = 'INSERT' THEN
		insert into linked.tag_helper (id, status)
		select new."assetsId", true;
	END IF;
	RETURN NULL;
END;
$$ LANGUAGE plpgsql;
...
-- RETURNS TRIGGER AS $$
-- BEGIN
-- if EXISTS (SELECT 1 FROM linked.asset WHERE id = new."assetId" and base_owner is true) and new."personId" is not null THEN
