...
with asset_filter as (select b.album_cluster,a.asset_cluster,b.owner_id,b.base_owner,a.id from linked.album as b
...
union
select b.tag_cluster,ls.owner_id,false,false,uuid_generate_v4() from base as b
left join linked.asset as ls using(asset_cluster)
where ls.base_owner is false;
...
left join linked.asset as a using(asset_cluster)
left join linked.tag as lt using(tag_cluster,owner_id)
where a.base_owner is false
...
--ALTER TABLE linked.asset_face ADD CONSTRAINT asset_face_person_fk FOREIGN KEY (person_id) REFERENCES linked.person(id);
...
--ALTER TABLE linked.album ADD CONSTRAINT album_album_fk FOREIGN KEY (id) REFERENCES public.album(id) ON DELETE CASCADE;
...
DECLARE
    a_cluster UUID;
BEGIN
    IF EXISTS (SELECT 1 FROM linked.asset WHERE id = old.id) THEN
...
BEGIN
	IF EXISTS (SELECT 1 FROM linked.asset WHERE id = old."assetsId") THEN
...
DECLARE
    t_cluster UUID;
BEGIN
    IF EXISTS (SELECT 1 FROM linked.tag WHERE id = old.id) THEN
...
DECLARE
    a_cluster UUID;
BEGIN
    IF EXISTS (SELECT 1 FROM linked.shared_album WHERE id = old.id) THEN
...
DECLARE
    t_cluster UUID;
BEGIN
    IF EXISTS (SELECT 1 FROM linked.tag WHERE id = new.id) THEN
...
DECLARE
    a_cluster UUID;
BEGIN
    IF EXISTS (SELECT 1 FROM linked.shared_album WHERE id = new.id) THEN
...
DECLARE
    a_cluster UUID;
BEGIN
    IF EXISTS (SELECT 1 FROM linked.asset WHERE id = new.id) THEN
...
DECLARE
    f_cluster UUID;
BEGIN
	IF EXISTS (SELECT 1 FROM linked.person WHERE id = new.id) THEN
		SELECT face_cluster INTO f_cluster FROM linked.person WHERE id = new.id;
		update person
		set "name"=new."name", 
		"isHidden"=new."isHidden", 
		"birthDate"=new."birthDate", 
		"thumbnailPath"=new."thumbnailPath",
		color=new.color
		where id in (select id from linked.person 
			where face_cluster = f_cluster and id != new.id);
	END IF;
	RETURN NULL;
END;
$$ LANGUAGE plpgsql;
...
CREATE OR REPLACE FUNCTION linked.link_asset_face()
RETURNS TRIGGER AS $$
BEGIN
if EXISTS (SELECT 1 FROM linked.asset WHERE id = new."assetId") and new."personId" is not null THEN
	if TG_OP = 'INSERT' and new."sourceType" = 'manual' THEN
		...
		final_asset_face as (
			select asset_cluster,face_cluster,owner_id,base_owner,asset_id,person_id,id from base
			union all
			select b.asset_cluster,b.face_cluster,ls.owner_id,ls.base_owner,ls.id as asset_id,p.id as person_id,uuid_generate_v4() as id from base as b
			left join linked.asset as ls using(asset_cluster)
...
BEGIN
	...
	IF EXISTS (SELECT 1 FROM linked.person WHERE id = new.person_id) THEN
...
BEGIN
	IF EXISTS (SELECT 1 FROM linked.asset WHERE id = new."assetsId") THEN
		IF NOT EXISTS (SELECT 1 FROM linked.shared_album WHERE id = new."albumsId") THEN
			--- new linked album
			with base as (select al."albumName" as album_name,uuid_generate_v4() as shared_album_cluster,la.asset_cluster,la.owner_id,la.base_owner,aaa."albumsId" as id from album_asset as aaa
				left join linked.asset as la on aaa."assetsId"=la.id
				left join album as al on aaa."albumsId"=al.id
				where aaa."albumsId"=new."albumsId" and aaa."assetsId"=new."assetsId"),
			final_album as (
				select album_name,shared_album_cluster,owner_id,base_owner,id from base
				union all
				select b.album_name,b.shared_album_cluster,ls.owner_id,ls.base_owner,uuid_generate_v4() from base as b
				left join linked.asset as ls using(asset_cluster)
			...
			FROM final_album as m
				...
				where m.id=new."albumsId" and n.id!=new."albumsId"),
			patched AS (
			  SELECT data || jsonb_build_object('id', to_jsonb(new_id), 
			  							'ownerId', to_jsonb(owner_id)) AS new_data
			  FROM joined),
...
BEGIN
	IF EXISTS (SELECT 1 FROM linked.asset WHERE id = new."assetsId") THEN
		IF NOT EXISTS (SELECT 1 FROM linked.tag WHERE id = new."tagsId") THEN
			--- new linked tag
			with base as (select uuid_generate_v4() as tag_cluster,la.asset_cluster,la.owner_id,la.base_owner, false as parent_updated,ta."tagsId" as id from tag_asset as ta
				left join linked.asset as la on ta."assetsId"=la.id
				where ta."tagsId"=new."tagsId" and ta."assetsId"=new."assetsId"),
			final_tag as (
				select tag_cluster,owner_id,base_owner,parent_updated,id from base
				union all
				select b.tag_cluster,ls.owner_id,ls.base_owner,false,uuid_generate_v4() from base as b
				left join linked.asset as ls using(asset_cluster)
				where ls.owner_id!=b.owner_id),
			joined AS (
				SELECT 
			...
			insert_linked_tag as (
				insert into linked.tag (tag_cluster,owner_id,base_owner,parent_updated,id)
				select tag_cluster,owner_id,base_owner,parent_updated,id from final_tag
				on conflict (id) do nothing
				RETURNING 1)
			---- insert into tag_closure
			INSERT INTO tag_closure (id_ancestor,id_descendant)
...
DECLARE
	...
	a_album_cluster uuid;
BEGIN
    IF new."facesRecognizedAt" is not null 
		and new."metadataExtractedAt" is not null 
		and new."duplicatesDetectedAt" is not null 
		and new."previewAt" is not null 
		and new."thumbnailAt" is not null 
		and NOT EXISTS (SELECT 1 FROM linked.asset WHERE id = new."assetId") THEN
		--- if asset should be linked
		SELECT uuid_generate_v4(), la.album_cluster
        INTO a_asset_cluster, a_album_cluster
        FROM asset pa
        JOIN linked.album la ON pa."originalPath" ILIKE la.description || '%'
        WHERE pa.id = NEW."assetId";
		--- not found
        IF NOT FOUND THEN
            RETURN NULL;
        END IF;
		---
		with asset_filter as (select a_album_cluster as album_cluster, a_asset_cluster as asset_cluster, a."ownerId" as owner_id, true as base_owner, a.id from asset as a where a.id=NEW."assetId"),
		...
		additional_faces as (select af.album_cluster,uuid_generate_v4() as asset_cluster,a."ownerId" as owner_id,true as base_owner,a.id from asset as a
			inner join (select distinct "assetId",album_cluster from asset_face as af
		...
		joined AS (
		    ...
		    where m.id = new."assetId" and n.id != new."assetId"
		),
		patched AS (
			SELECT data || jsonb_build_object('id', to_jsonb(new_id), 
		  							'ownerId', to_jsonb(owner_id)) AS new_data
...
BEGIN
    IF new.base_owner is true THEN 
		...
		joined AS (
			...
			where m.asset_id = new."assetId" and n.asset_id != new."assetId"),
		patched AS (
			SELECT data || jsonb_build_object('id', to_jsonb(new_id), 
		  								'ownerId', to_jsonb(owner_id), 
		  								'assetId', to_jsonb(asset_id)) AS new_data
...
RETURNS TRIGGER AS $$
BEGIN
    IF new.base_owner is true and EXISTS (SELECT 1 FROM linked.asset WHERE id = new.id) THEN
		WITH joined AS (
			SELECT 
		    n.id as new_id,
		    to_jsonb(t) AS data
